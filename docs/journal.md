...
- Branche `demenagement` : le test « sans adresse » servait l'état du dépôt et deviendrait rouge dès l'adresse renseignée (bloquant la procédure `main`) ; il fixe désormais lui-même l'adresse vide (commit séparé sur cette branche).
...
- `tests/e2e/audit-independant.spec.mjs` : 21 tests ; **23 mutants tués sur 23** (dont, en seconde passe : reconstruction de fin de rafale, file non attendue à la réouverture, cause unique pour tous, nom pris sur l'élève courant, toast en double, ancien niveau non éteint, score non mis à jour, mesure au seul redimensionnement, consigne réservée aux points ajustables, bandeau multi-lignes). Suite 272 (+ 72 mobile).
