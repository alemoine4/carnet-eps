...
|---|---|---|---|
| v0.13.4 | `v0.13.4` | *(à compléter au commit)* (branche `grilles-schema3`) | **Saisie par grille « un élève à la fois » sur un seul écran de téléphone** (retour de l’essai) : niveaux sur une rangée, barre « Élève précédent / suivant » collante sous le pouce, élève suivant ramené en haut, actions de bureau sous la saisie ; 112 px par élève en mode « Par critère » (environ 270 avant) ; barre FIXE sous le pouce avec ligne d’erreur persistante par élève et critère, bande sous la barre neutralisée (appel compris), annonce du nouvel élève, noms de niveau jamais coupés ; trois revues adversariales (14, 19 puis 14 constats), tous corrigés ; CI étendue à `grilles-schema3` ; 11 tests, 38 mutants tués — aucun changement de données |
