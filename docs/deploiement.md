...
|---|---|---|---|
| v0.12.22 | `v0.12.22` | *(à compléter au commit)* (branche `demenagement`) | **Déployée le 2026-09-17** avec `NOUVELLE_ADRESSE = 'https://carnet-eps.github.io/'`, en même temps que la v0.13.3 sur la nouvelle adresse. | **Bandeau « Carnet EPS a déménagé »** pour l’ANCIENNE adresse, à publier une fois la nouvelle installation en place (audit A01) : renseigner l’adresse HTTPS dans `app/js/demenagement.js` avant le déploiement ; 5 tests, suite 221 — **préparée, NON déployée** |
| v0.12.21 | `v0.12.21` | `aaf1539` | **Statuts d’appel distinguables** (retour de terrain du 2026-09-16), reportés de la v0.13.2 : retard en pastille jaune vif, oubli de tenue en magenta, dispense en indigo ; écart perceptuel ≥ 34 entre les statuts les plus lus, ≥ 18 sur la palette, contrastes conservés ; même pastille dans la fiche élève ; 4 tests (dont le thème sombre automatique), 5 mutants tués, suite 220 — schéma 2 inchangé, version réversible — déployée le 2026-09-16 (gh-pages `8578c2f`) |
