...
|---|---|---|---|
| v0.13.5 | `v0.13.5` | *(à compléter au commit)* (branche `grilles-schema3`) | **Correctifs de l’audit Codex V7** sur la saisie par grille : un avertissement « Non enregistré » déjà rattrapé ne revient plus (l’échec d’un élève passé « parti » est gardé), et l’écran ne défile plus sous le doigt au choix d’un statut ABS / DISP / NN (modalité doigt / clavier suivie à un seul endroit pour la vue) ; deux tours de revue adversariale ; 52 mutants tués sur 52 — aucun changement de données — **candidate, NON publiée** |
