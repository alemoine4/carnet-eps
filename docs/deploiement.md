...
> Dépôt : https://github.com/alemoine4/carnet-eps · **App en ligne : https://carnet-eps.github.io/** (v0.13, schéma 3, mise en service le 2026-09-17) · Ancienne adresse : https://alemoine4.github.io/carnet-eps/ (v0.12.22, bandeau « a déménagé », à ne plus utiliser une fois les données migrées)
...
**Mise en service de la nouvelle adresse** (après l'essai téléphone, AVANT tout import de vraies données) — ✅ **faite le 2026-09-17** : v0.13.3 sur la nouvelle adresse, v0.12.22 sur l'ancienne ; reste l'étape 3, appareil par appareil :
...
|---|---|---|---|
| v0.14.1 | `v0.14.1` | `6d7ca86` (branche `grilles-schema3`, commit du site `3819718` ; code de l’app identique à `afbec3c`) | **Marqueurs de séance, le vocabulaire** : écran `#/marqueurs` (Plus → Marqueurs de séance) — liste par genre, archivés en dernier, aperçu du code, formulaire (libellé, code court en majuscules, genre fixé à la création, couleur), archiver / restaurer, aucune suppression ; amorçage des 6 marqueurs proposés sur un vocabulaire vide, échec partiel dit ; doublons de code d’une sauvegarde bricolée nommés, avec l’archivage de l’un à portée de main ; `ASSETS` += `./js/modules/marqueurs.js` ; aucun changement de format ni de `DB_VERSION` (4) ; revue adversariale avant commit (8 constats corrigés : code court jamais réécrit pendant la frappe, repère neutre dans l’aperçu d’un comportement, focus gardé dans la vue) ; 5 tests (`marqueurs.spec.mjs` : MQ-12, MQ-17, MQ-18, MQ-19, MQ-20), 29 mutants tués par leur test ; écran regardé au téléphone émulé, clair et sombre (l’aperçu d’un comportement garde son repère à côté de « Sur la carte : ») ; **publiée et vérifiée en ligne le 2026-09-25**, par 4 vérifications indépendantes : les 33 fichiers servis identiques octet pour octet au commit du site (avec et sans cache), les 32 entrées d’`ASSETS` servies et précachées, `io.js` et `marqueurs-calcul.js` inchangés depuis la v0.14.0 ; premier visiteur (Pixel 7, contexte neuf) : 0 erreur, base en schéma 4 à 17 magasins, parcours Plus → amorçage → formulaire ; **mise à jour d’un appareil en v0.14.0 avec données** (même origine, rejouée en local) : toast « Nouvelle version installée. » puis « Recharger », service-worker 0.14.1, ancien cache supprimé, données identiques, écran lisible hors ligne ; l’ancienne adresse reste en v0.12.22, schéma 2, sans `marqueurs.js` |
| v0.14.0 | `v0.14.0` | `7eadf60` (branche `grilles-schema3`, commit du site `54aa119` ; code de l’app identique à `ca99522`) | **Marqueurs de séance, le format seul — schéma 4** : magasins `marqueurs` et `marquages`, écritures `appliquerMarquages` / `ecrireMarqueur`, sauvegardes de schéma 4 (schémas 1 à 3 toujours restaurables), cascades et aperçus (dont l’aperçu neuf de la suppression d’une séance), message en français devant une base plus récente ; aucun écran nouveau ; 11 tests (`marqueurs-migration.spec.mjs`, MIG-07 élargi à 15 cas après la revue adversariale), 23 mutants tués (25 exécutions avec les variantes), sonde de C37 dérivée de la base, suite 392/392 sur les deux projets ; **publiée et vérifiée en ligne le 2026-09-23** (fichiers servis en 0.14.0, `marqueurs-calcul.js` servi et précaché, page contrôlée par le service-worker, base montée en **schéma 4** avec ses 17 magasins ; l’ancienne adresse reste en v0.12.22, schéma 2). ⚠ **Montée de schéma sans retour** : une fois publiée, plus jamais de version en `DB_VERSION = 3` (voir « Côté utilisateur, après un rollback ») |
| v0.13.5 | `v0.13.5` | `f8fa34b` (branche `grilles-schema3`, commit du site `6584871`) | **Correctifs de l’audit Codex V7** sur la saisie par grille : un avertissement « Non enregistré » déjà rattrapé ne revient plus (l’échec d’un élève passé « parti » est gardé), et l’écran ne défile plus sous le doigt au choix d’un statut ABS / DISP / NN (modalité doigt / clavier suivie à un seul endroit pour la vue) ; deux tours de revue adversariale ; 52 mutants tués sur 52 — aucun changement de données — publiée et vérifiée en ligne le 2026-09-22 (fichiers servis et page contrôlée par le service-worker en 0.13.5 ; l’ancienne adresse reste en v0.12.22, schéma 2) |
| v0.13.4 | `v0.13.4` | `ea5e3ec` (branche `grilles-schema3`, commit du site `90e823b`) | **Saisie par grille « un élève à la fois » sur un seul écran de téléphone** (retour de l’essai) : niveaux sur une rangée, barre « Élève précédent / suivant » FIXE sous le pouce, élève suivant ramené en haut, actions de bureau sous la saisie ; 112 px par élève en mode « Par critère » (environ 270 avant) ; barre FIXE sous le pouce avec ligne d’erreur persistante par élève et critère, bande sous la barre neutralisée (appel compris), annonce du nouvel élève, noms de niveau jamais coupés ; quatre revues adversariales (14, 19, 14 puis 8 constats, dont un bloquant trouvé juste avant publication : l’écran sautait sous le doigt et une note partait sur un autre élève), tous corrigés ; CI étendue à `grilles-schema3` ; 12 tests, 45 mutants tués — aucun changement de données ; publiée et vérifiée en ligne le 2026-09-17 (l’ancienne adresse reste en v0.12.22, schéma 2) |
| v0.13.3 | `v0.13.3` | `dc67ad5` (branche `grilles-schema3`, commit du site `0f1176c`) | **Mise en service de la nouvelle adresse** après validation de l’essai téléphone : `MODE_ESSAI = false`, manifeste « Carnet EPS » ; publiée en même temps que la v0.12.22 (bandeau « a déménagé », `eb2b31e`, gh-pages `aca10d2`) sur l’ancienne adresse ; vérifiées en ligne le 2026-09-17 — schéma 3 inchangé |
...
- ⚠ **Schéma 4 (v0.14.0)** : un appareil qui a ouvert la v0.14.0 a sa base en schéma 4 (magasins `marqueurs` et `marquages`). **Ne jamais redéployer une version dont `DB_VERSION` est 3** (v0.13.0 à v0.13.5) : sur cet appareil, `indexedDB.open('carnet-eps', 3)` échoue en `VersionError`, l’écran affiche « Affichage impossible » et **aucune donnée ne s’affiche**. Rien n’est effacé pour autant — le navigateur refuse d’ouvrir, il ne vide rien — mais l’application est inutilisable, et une sauvegarde de schéma 4 est refusée par une v0.13 avant toute écriture. **Un correctif après la v0.14.0 se fait donc TOUJOURS en avant** : garder `DB_VERSION = 4` et les deux magasins.
  - **Procédure de retour arrière 4 → 3** (si un défaut l’exige malgré tout) : (1) **d’abord et de préférence**, un correctif en avant — `git revert` du commit fautif sur `grilles-schema3`, `DB_VERSION` laissé à 4, nouveau numéro de version, suite verte, publication ; c’est la seule voie qui n’exige rien des appareils. (2) S’il faut vraiment retrouver le comportement de la v0.13.5, republier son code **en gardant `DB_VERSION = 4` et les deux entrées `marqueurs` / `marquages` de `SCHEMA`** (le code ancien les ignore) sous un nouveau numéro : un code en `DB_VERSION = 3` ne s’ouvrirait sur aucun appareil déjà passé en 4. (3) Remettre un appareil en schéma 3 n’est possible qu’à la main et **en perdant tous les marqueurs** : sauvegarde complète depuis la v0.14, effacement des données du site dans le navigateur, puis import dans la version en schéma 3 — qui refuse une sauvegarde de schéma 4 (`schéma 4 > 3`) tant qu’on n’a pas retiré `marqueurs` et `marquages` du fichier et ramené `schemaVersion` à 3. C’est précisément ce que la montée de schéma rend impossible par construction (décision 17) : à éviter.
  - **Ce que voit un appareil resté en v0.13.5** (service-worker pas encore à jour) quand sa base a déjà été ouverte en schéma 4, par exemple dans un autre onglet : l’ouverture est refusée par le navigateur, la carte « Affichage impossible » s’affiche avec le texte anglais de la `DOMException`, et le conseil « exportez une sauvegarde » y est **inapplicable** (la base ne s’ouvre pas). **Aucune donnée n’est perdue. La seule sortie : recharger la page en ligne** pour recevoir la version à jour (`index.html` est revalidé en réseau). La v0.13.5 étant déjà publiée, aucune version future ne peut rendre son message plus clair ; à partir de la v0.14.0, ce cas affiche « cet appareil a déjà ouvert le carnet avec une version plus récente de l’application — rechargez la page ».
