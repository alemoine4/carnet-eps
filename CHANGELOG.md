...
- **Marge de focus sous l'en-tête** : elle suit la hauteur réelle de l'en-tête, mesurée en continu (le bandeau d'essai l'agrandissait, et une case atteinte au clavier pouvait finir cachée dessous à 320 px, y compris texte agrandi). Le bandeau tient sur une ligne, même texte agrandi à 200 %.
...
- **Deux revues adversariales du lot** (lentilles indépendantes, 2 réfutateurs par constat) : la première a retenu 18 constats sur 19, la seconde — sur les corrections de la première — 17 sur 20 ; tous corrigés ci-dessus. Parmi eux, la promesse « un double tap reste idempotent » de la première version de ce lot (vraie seulement si le second tap arrivait pendant l'écriture), puis la reconstruction de fin de rafale introduite par sa correction.
- Tests : `tests/e2e/audit-independant.spec.mjs` (21), 23 mutants tués, suite 272 (+ 72 rejoués sur mobile).
