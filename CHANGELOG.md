...
- **Un nom de niveau n'est jamais coupé au milieu d'un mot** : si un mot ne tient pas dans sa case (libellé long, grande police, police plus large du téléphone), les cases s'élargissent — moins de colonnes, puis une seule — avant d'en venir à couper. La mesure se fait sur l'écran réel, à chaque rendu et à chaque rotation.
- **« Élève précédent / Élève suivant » dans une barre FIXE sous le pouce**, au-dessus de la navigation, **toujours à la même place** quel que soit le défilement : un double tap ne tombe plus sur une case de l'élève suivant. Passer à l'élève suivant, ou en choisir un dans la liste (au doigt ou à la souris), ramène son nom **en haut de l'écran** ; au clavier, la liste reste visible. Même chose pour « Critère suivant ». Quand ses BOUTONS prendraient plus de 15 % de la hauteur (texte agrandi, paysage, écran partagé), elle redevient un bloc ordinaire en fin de saisie ; la ligne d'erreur, elle, ne la fait jamais basculer. Les messages (« Nouvelle version installée », « Annuler ») passent au-dessus d'elle ; quand elle bascule en cours de saisie, les lecteurs d'écran l'annoncent, et une marge évite les allers-retours. Une case atteinte **au clavier** reste visible quand la barre grandit — **au doigt, rien ne fait défiler l'écran** : un tap ne doit jamais déplacer l'écran sous la main.
- **Une écriture refusée reste affichée dans la barre** (« Non enregistré — cause : NOM Prénom · critère », deux lignes au plus, **la cause d'abord** parce que c'est elle qui dit quoi faire ; au-delà de deux échecs la ligne résume, le détail complet restant dans l'infobulle) tant que CE choix n'a pas été réécrit avec succès : ni un autre critère, ni un autre élève ne l'effacent, et rien ne recouvre les boutons. Elle **survit au rechargement** que le message de conflit demande parfois, et disparaît si le choix a été enregistré entre-temps. Un statut refusé est nommé (« statut ABS »). Avant, l'erreur restait au-dessus de la saisie, hors de l'écran dès le deuxième élève, et le succès suivant l'écrasait. Une seule annonce pour les lecteurs d'écran.
...
- **Quatre revues adversariales**, chacune sur les correctifs de la précédente (2 réfutateurs par lentille) : 14 constats retenus sur 15, puis 19 sur 20, puis 14 sur 14, puis 8 sur 8 — mes correctifs avaient successivement posé un message de 8 s sur la barre, laissé la barre bouger depuis le bas de page, rendu le retour en haut sans effet au doigt, laissé un VRAI message d'erreur faire basculer la barre hors du pouce, et enfin — **défaut bloquant, trouvé juste avant publication** — fait sauter l'écran de plusieurs centaines de pixels sous le doigt en ramenant le focus en vue : le tap suivant écrivait alors une note maximale pour un AUTRE élève. Tous corrigés.
- Tests : 11 dans `tests/e2e/grilles.spec.mjs` (aussi rejoués sur le profil mobile), 1 dans `tests/e2e/terrain.spec.mjs` (bande sous « Terminer l'appel »), assertions FON-05 portées sur l'état des cases, la ligne d'erreur de la barre et le statut refusé. Les mesures se font avec les VRAIS messages d'erreur (conflit entre onglets, stockage plein) et un nom composé, aux tailles où ils coûtent le plus, et l'écran ne doit bouger ni sous un tap, ni à l'apparition d'une erreur, ni au redimensionnement. Libellés de niveau courts dans les tests d'écran, et hauteurs d'écran déduites de la barre réellement rendue : la suite ne dépend pas de la police installée (vérifiée aussi sous Linux en intégration continue). 45 mutants tués.
...
- **La version d'essai se distingue de la production (FON-01).** Deux applications visuellement identiques circulaient : une saisie réelle faite dans l'essai, ou fictive dans la production, créait deux carnets irréconciliables. Un bandeau ambre « Version d'essai — n'y saisissez que des données fictives » vit dans l'en-tête, le titre de l'onglet commence par « ESSAI », et l'application s'appelle « EPS essai » si elle est quand même installée. Un seul drapeau (`MODE_ESSAI`, `state.js`) pilote le tout ; un test de cohérence exige que le manifeste change avec lui, champ par champ. **L'ordre de la migration est fixé en conséquence** : le drapeau passe à false et se publie AVANT le premier import de vraies données, jamais après (`docs/deploiement.md`).
- **Saisie par grille : plus aucun tap perdu (FON-05, PER-05).** Un tap arrivé pendant l'enregistrement du précédent était jeté — sans que rien ne se voie, aucun style de bouton désactivé n'existant. Les écritures sont désormais **sérialisées** (même motif que les notes), dans une file **partagée** entre les ouvertures successives de l'écran (quitter puis rouvrir la grille pendant une rafale relisait un état intermédiaire et faisait refuser le tap suivant « autre onglet »). Chaque tap est **jugé sur l'état laissé par les taps précédents**, ni sur l'écran figé ni sur la base : deux taps rapides sur deux critères du même élève se cumulent, changer d'avis dans la même rafale garde le dernier choix, et « retoucher efface » donne le même résultat que le second tap arrive pendant ou après l'écriture du premier, quelle que soit la vitesse du téléphone. Conséquence à connaître : un double tap sur une case non choisie la choisit puis l'efface — le critère repasse « Non évalué », niveau précédent compris ; sur la case déjà choisie, il l'efface puis la rechoisit, rien ne change. Cette règle est maintenant **affichée pour toutes les grilles** (elle ne l'était qu'avec les points ajustables). La ligne touchée montre le choix **dès le tap** ; l'écriture en cours est signalée par un contour pointillé, visible aussi sur une case sélectionnée, au clavier et sur le sélecteur ABS/DISP/NN ; rien n'est verrouillé. **En fin de rafale, l'écran se met à jour sur place** au lieu d'être reconstruit : la reconstruction détachait la case qu'on touchait (un appui long commencé pendant l'écriture n'ouvrait jamais « Ajuster »), celle qu'on avait sous la souris (clic perdu) et reprenait le focus à celle où l'on était allé au clavier. Une erreur nomme chaque élève concerné avec sa propre cause (on a pu passer au suivant entre-temps), n'est pas recouverte par le succès suivant, et s'affiche en message si la vue a été quittée. La feuille « Ajuster » ouverte pendant une écriture présente la valeur voulue.
- **Marge de focus sous l'en-tête** : elle suit la hauteur réelle de l'en-tête, mesurée en continu (le bandeau d'essai l'agrandissait, et une case atteinte au clavier pouvait finir cachée dessous à 320 px, y compris texte agrandi). Le bandeau reste lisible en entier, sur une ligne aux largeurs de téléphone et sur deux au plus en texte agrandi.
- **Les boutons désactivés se voient enfin** : style `:disabled` global (opacité, curseur), qui manquait à toute l'application.
- **Intégration continue (SEC-04), en défense en profondeur** : le jeton du workflow est déclaré en lecture seule (`permissions: contents: read`) et les trois actions sont épinglées par SHA de commit au lieu d'un tag mobile — les SHA retenus sont ceux que `v4` désigne aujourd'hui, donc aucun changement du code exécuté. **Correction d'une prémisse de l'audit** : le jeton n'était pas « né avec des droits d'écriture » ; le réglage du dépôt était déjà en lecture seule (vérifié par l'API GitHub). Le bloc maintient cette lecture seule si le réglage change un jour. Le durcissement prend effet en intégration continue à la fusion dans `main`, seule branche qui la déclenche.
- **Procédures de la période à deux adresses** (`docs/deploiement.md`) : la seule procédure documentée — publication comme retour arrière — publiait sur l'ancienne adresse, ce qui aurait fait passer les vraies données en schéma 3 sans retour. Publication et retour arrière de l'origine d'essai, avertissement en tête du document, ordre de mise en service.
- **Fiche d'essai** : `docs/essai-telephone.md` (15 min, données fictives, sans installer, sans coller dans Pronote).
- **Trois revues adversariales du lot**, chacune sur les corrections de la précédente (lentilles indépendantes, 2 réfutateurs par constat) : 18 constats retenus sur 19, puis 17 sur 20, puis 5 sur 6 ; tous corrigés ci-dessus. Parmi eux, trois défauts que mes propres correctifs avaient introduits : la promesse « un double tap reste idempotent » (vraie seulement si le second tap arrivait pendant l'écriture), la reconstruction de l'écran en fin de rafale, et un bandeau « sur une seule ligne » qui tronquait « données fictives » dès 320 px.
- Tests : `tests/e2e/audit-independant.spec.mjs` (22), 25 mutants tués, suite 273 (+ 73 rejoués sur mobile). Quatre tests ne passaient que grâce au ralentissement de la capture de trace : ils attendent désormais le rendu de la vue.
...
- **Export Pronote** : les valeurs partent sur le barème choisi, avec les protections de la v0.12.20 (attente des écritures, relecture de la base, publication « à remettre à jour »).
- **Une note de grille calculée sur une partie des critères est signalée à la copie.** L'écran de la grille affichait « 2/4 critères évalués », la copie vers Pronote rien : un 15 calculé sur deux critères partait comme un 15 complet. Le récapitulatif la liste désormais, comme les codes à saisir à la main, et la ligne d'état de la copie le mentionne. Avec la règle « compter comme zéro », le signalement dit combien de critères ont été comptés zéro.
- **Le piège du barème est levé (V3-B3).** Changer le barème d'une évaluation déjà notée gardait les points en silence : 8/10 devenait 8/20. L'application demande maintenant ce que deviennent les notes : **convertir** (8/10 devient 16/20, arrondi au centième), **garder les points**, ou **annuler**. Les codes ABS, DISP et NN ne bougent pas, la conversion s'écrit dans la même transaction que le barème, et garder des points au-dessus du nouveau barème reste refusé. Sans aucune note chiffrée, aucune question n'est posée.
...
- **Correction de la classe entière**, pas du seul symptôme : l'objet métier n'est modifié qu'**après** une écriture validée, dans les cinq écrans concernés (évaluation, séquence, fiche élève, fiche classe, inaptitude, bilan de séance) — une vingtaine de champs. C'est l'ordre que la saisie d'une note appliquait déjà.
- **Deux champs qui n'avaient aucun filet en reçoivent un** : la couleur d'une classe et les cases « restrictions » d'une inaptitude s'écrivaient sans rien dire en cas de refus. Elles affichent désormais un message et reviennent à leur état enregistré.
- **Deux sites que l'audit n'avait pas vus** sont corrigés au passage : le marquage « publiée » (après une copie Pronote et par le bouton manuel) suivait le même motif. Une écriture refusée pouvait donc inscrire plus tard une publication qui n'avait jamais eu lieu.
- Origine : audit Codex V3 du 2026-09-09 (constat V3-01), vérifié indépendamment ; stratégie complète dans `docs/strategie-audit-v3.md`. Le défaut avait été introduit dans sa forme la plus visible la veille, par le champ « barème modifiable » de la v0.12.14.
- **Revue adversariale du diff** (3 lentilles + 2 réfutateurs par constat) : deux défauts corrigés dans la version — le bouton **« Archiver »** d’une classe (et la restauration d’une classe archivée) gardait le motif fautif, et la correction elle-même **réintroduisait une course** : deux champs modifiés coup sur coup construisaient chacun leur candidat depuis l’objet d’avant, et le second écrasait le premier. Les écritures d’une même vue sont désormais **sérialisées**.
- Vérifié : **9 tests** (`tests/e2e/audit-v3.spec.mjs`), chacun **rouge avant / vert après** — chaque test refuse un champ, modifie un champ voisin, et exige que la valeur refusée n'apparaisse jamais en base ; suite complète **174 tests** (+ 24 rejoués sur le profil tactile). Bump SW + `VERSION_APP` → 0.12.15.
