...
- **Trois revues adversariales**, chacune sur les correctifs de la précédente (2 réfutateurs par lentille) : 14 constats retenus sur 15, puis 19 sur 20, puis 14 sur 14 — mes correctifs avaient successivement posé un message de 8 s sur la barre, laissé la barre bouger depuis le bas de page, rendu le retour en haut sans effet au doigt, puis laissé un VRAI message d'erreur (long) faire basculer la barre hors du pouce. Tous corrigés.
- Tests : 10 dans `tests/e2e/grilles.spec.mjs` (aussi rejoués sur le profil mobile), 1 dans `tests/e2e/terrain.spec.mjs` (bande sous « Terminer l'appel »), assertions FON-05 portées sur l'état des cases, la ligne d'erreur de la barre et le statut refusé. Les mesures se font avec les VRAIS messages d'erreur (conflit entre onglets, stockage plein) et un nom composé, aux tailles où ils coûtent le plus. Libellés de niveau courts dans les tests d'écran, pour ne pas dépendre de la police installée. 38 mutants tués.
