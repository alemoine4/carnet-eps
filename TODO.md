...
- [x] 2026-09-09 — **Lot 2 livré en v0.12.13** (avis validé par délégation) : créations atomiques (V2-06, D-04, D-06) — inaptitude avec pièce, remplacement de pièce, document, photo (ajout, changement, retrait) et import CSV écrits chacun en UNE transaction ; socle `preparerFichier` + `enregistrerLot` ; **intégration continue** GitHub Actions (C64). 5 tests (`tests/e2e/audit5-lot2.spec.mjs`), suite 158/158.
- [ ] **v0.12.14 (en cours)** : points retenus de l’avis du lot 5 (C15 barème modifiable, A34 (a), C45 (b), A31 (a), A33 plafond d’import, D-08 (3) taille des pièces, C60 projet mobile Playwright) + **A39** (index.html servi depuis le cache) ; écartés : C44 (1) et C16 (2).
- [ ] ⚙ **Avis « grilles d’évaluation EPS »** (demandé le 2026-09-09) : critères, niveaux de maîtrise, pondérations, total ramené sur 20 avec arrondi configurable, export Pronote au barème choisi, traitement des critères non évalués — **à rédiger avant tout code**.
