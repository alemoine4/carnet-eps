...
## Phase active (2026-09-17) : ✅ **mise en service faite** (v0.13.3 sur https://carnet-eps.github.io/, v0.12.22 avec bandeau « a déménagé » sur l’ancienne adresse) → **étape 5 = migration de TES données, appareil par appareil** (`docs/guide-installation.md`, « Déménagement »). Historique de la période à deux adresses : v0.13.2 publiée sur l’origine d’essai https://carnet-eps.github.io/ (v0.13.1 : bandeau « version d’essai », grille sans tap perdu, CI durcie, premier lot de l’**audit indépendant du 2026-09-16**, rapport hors dépôt, 36 constats, plan d’action en §6 ; v0.13.2 : retours de l’essai, statuts d’appel distinguables et écran des grilles aéré) ; l’ancienne adresse est en v0.12.21 (même palette des statuts). **Étape 3 = essai téléphone** (`docs/essai-telephone.md`, données fictives, sans installer) → étape 4 = mise en service : `MODE_ESSAI = false` + manifeste publiés sur la nouvelle adresse, et bandeau « a déménagé » publié sur l’ancienne (branche `demenagement`) → étape 5 = SEULEMENT ENSUITE, migration des données appareil par appareil (procédure : `docs/deploiement.md`, « Période à deux adresses »).

- [x] 2026-09-23 — **v0.14.0 publiée** sur https://carnet-eps.github.io/ : le format des marqueurs de séance (schéma 4, magasins `marqueurs` et `marquages`), sans écran. Base de l’enseignant montée en schéma 4 à sa prochaine ouverture ; elle était vide. **Suite : v0.14.1** (écran du vocabulaire + amorçage des 6 marqueurs proposés).
- [x] 2026-09-22 — **v0.13.5 publiée** sur https://carnet-eps.github.io/ : correctifs de l’audit Codex V7 (échec rattrapé qui ressuscitait, écran qui défilait sous le doigt au statut ABS).
- [x] 2026-09-17 — **v0.13.4** : saisie par grille « un élève à la fois » sur un seul écran de téléphone (retour de l’essai) — niveaux sur une rangée, barre fixe « Élève précédent / suivant », 112 px par élève en mode « Par critère ».
- [ ] ⚙ **Avis « marqueurs de séance » RÉDIGÉ le 2026-09-17** : `docs/avis/AVIS_MARQUEURS_SEANCE.md` (demande : « des infos comme score, groupe » puis « système de pastilles pour des rôles dans la séance ou pour comportement », référence iDoceo) — **RÉVISÉ le 2026-09-22** après la contre-vérification Codex V7 (3 risques majeurs ajoutés : anciennes versions qui perdent les marqueurs, présence validée en douce, vues concurrentes) — ✅ **RÉPONDU le 2026-09-22 : « je te suis », 18 réponses actées** (les marqueurs AVANT le carnet à onglets ; magasin dédié + `DB_VERSION` 4 à figer avant la première classe saisie) → contrat d’implémentation `docs/avis/AVIS_FORMAT_MARQUEURS.md` RÉDIGÉ le 2026-09-23 (3 tours de réfutation), ✅ **VALIDÉ le 2026-09-23 (« je te suis » aux 11 points)** → **v0.14.0 « le format seul » LIVRÉE en candidate le 2026-09-23, NON publiée, NON commitée** (schéma 4 : magasins `marqueurs` et `marquages`, `appliquerMarquages`, `ecrireMarqueur`, sauvegardes, cascades et aperçus, 11 tests MIG, 13 mutants tués — voir `docs/journal.md`). **Prochaines étapes** : ta relecture et ton « go » de commit puis de publication (⚠ publier = montée de schéma SANS retour) ; puis v0.14.1 (vocabulaire + amorçage des 6 marqueurs), v0.14.2 (poser et relire, « Terminer l’appel » durci), v0.14.3 (mode tampon), v0.14.4 (reprise). Le score chiffré reste aux évaluations ; le carnet à onglets vient après.
- [ ] ⚙ **Avis « carnet de classe à onglets » RÉDIGÉ le 2026-09-16** : `docs/avis/AVIS_PAGE_CLASSE_ONGLETS.md` (demande : « fonctionner avec des onglets en partant d’une classe », référence iDoceo) — **EN ATTENTE DE TES 14 RÉPONSES** (§12). Recommandation : onglets **déduits de l’existant** d’abord (Classe · Présences · Évaluations, liste « Feuille » par séquence ou trimestre, v0.14.0 → v0.14.3, rien à stocker, corrige FON-02 et FON-03), onglets **personnels** ensuite (v0.15, champ facultatif `classes.onglets`, avis de format séparé). **Rien ne commence avant la fin de la migration des vraies données.**
...
- [ ] 🔬 **Audit Codex V3 du 2026-09-09** (`audit codex/AUDIT_V3.md`, hors dépôt) : 5 constats P2/P3 reproduits sur v0.12.14 — **stratégie écrite : `docs/strategie-audit-v3.md`**, 3 familles, 6 lots, 3 versions. **En attente de ta réponse sur les décisions 1, 2 et 3** (§5). ✅ **Lot V3-A livré en v0.12.15** (mutation après écriture validée : 5 modules, ~20 champs, 2 champs sans filet équipés, 2 sites de marquage « publiée » et 2 boutons d archivage corrigés ; 9 tests dont une garde de classe ; déployée : main `92f6fa4`, gh-pages `ddf77ae`, tag `v0.12.15`). Reste la **famille B** (V3-B1/B2/B3), puis **V3-C1** et **V3-D**. ⚠ V3-01 et V3-05 sont des régressions des v0.12.14.
